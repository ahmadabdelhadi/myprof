...
//     if (!$request->user()->status=='ملغي') {
//         // Store a message in the session
//         session()->flash('message', 'You must be approved by an admin before you can register.');

//         // Redirect the user back to the registration form
//         return redirect()->back();
//     }

    // The user has been approved, so proceed with the registration process...
// }
...
// }
// elseif($search &&auth::user()->role_id=='1' )
//     {
//         $user=auth()->user();
...
//     else{
//         $data =Teachers::where('teacher_Major','Like','%'.$search.'%')->orwhere('teacher_Name','Like','%'.$search.'%')->get();
//         return view('teacher.teachers_Page',compact('data'));
...
//     $user=auth()->user();
//     $data=User::find($id);
//     $cart=Cart::where('to_id',$user->id);
//     return view('chatify.php/id', compact('data','cart','id'));  
// return redirect()->route('chatify', ['id' => $id]);
...
//       $user=auth()->user();
//     $data=User::find($id);
//     $cart=Cart::where('to_id',$id)->first();
//         return view('chatify', compact('data','cart','id'));  
...
//     // return redirect()->route('chatify')->with($id);
//     if(auth::user()->role_id=='2' )
//     {
//         return redirect()->route('chatify', [ $id]);
//     }
