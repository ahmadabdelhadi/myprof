...
// نعمل متغير اسمه داتا ونخزن فيه المعلومات الي  خزنناها في جدول المعلمين في الداتا بيز
    ...
    return redirect()->back()->with('message','Teacher Add Successfully');//بعمل مثل الريفرش وبرجعه لنفس الصفحة وبمسح البيانات الي عبيتها لتعبي غيرها 
...
// هدول الفنكشن عشان الادمن يقبل ويلغي اليوزر
...
public function search_user(Request $request )
{
            ...
            $user=user::where('name','Like','%'.$search_user.'%')->orwhere('city','Like','%'.$search_user.'%')->orwhere('address','Like','%'.$search_user.'%')->orwhere('email','Like','%'.$search_user.'%')->orwhere('phone','Like','%'.$search_user.'%')->get();
            return view('admin.showall',compact('user'));
...
public function search_request(Request $request )
{
    ...
    $cart=cart::where('lesson_price','Like','%'.$search_request.'%')->orwhere('city','Like','%'.$search_request.'%')->
